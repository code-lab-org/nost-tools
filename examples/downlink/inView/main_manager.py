...
if __name__ == "__main__":
    ...
    manager.start_up("InView", config, True)

    # execute a test plan
    manager.execute_test_plan(
        datetime(2023, 5, 1, 12, 0, 0, 0, tzinfo=timezone.utc),                    # scenario start datetime
        datetime(2023, 5, 2, 0, 0, 0, 0, tzinfo=timezone.utc),                   # scenario stop datetime
        start_time=None,                                                        # optionally specify a wallclock start datetime for synchronization
        time_step=timedelta(seconds=1) * 300,                                         # wallclock time resolution for simulation
        time_scale_factor=300,                                                # initial scale between wallclock and scenario clock (e.g. if SCALE = 60.0 then  1 wallclock second = 1 scenario minute)
        time_scale_updates=[],                                              # optionally schedule changes to the time_scale_factor at a specified scenario time
        time_status_step=timedelta(seconds=1) * 300,                           # optional duration between time status 'heartbeat' messages
        time_status_init=datetime(2023, 5, 1, 12, 0, 0, 0, tzinfo=timezone.utc),   # optional initial scenario datetime to start publishing time status 'heartbeat' messages
