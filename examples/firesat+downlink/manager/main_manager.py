...
if __name__ == "__main__":
    ...
    manager.execute_test_plan(
        datetime(2023, 8, 7, 7, 20, 0, tzinfo=timezone.utc),                    # scenario start datetime
        datetime(2023, 8, 8, 7, 20, 0, tzinfo=timezone.utc),                   # scenario stop datetime
        start_time=None,                                                        # optionally specify a wallclock start datetime for synchronization
        time_step=timedelta(seconds=3),                                         # wallclock time resolution for simulation
        ...
        time_status_step=timedelta(seconds=0.33)* SCALE,                           # optional duration between time status 'heartbeat' messages
        time_status_init=datetime(2023, 8, 7, 7, 21, 0, tzinfo=timezone.utc),   # optional initial scenario datetime to start publishing time status 'heartbeat' messages
