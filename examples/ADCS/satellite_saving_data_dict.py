...
t_end = datetime.fromtimestamp(PARAMETERS["SCENARIO_START"]).replace(
    tzinfo=utc
...
class Satellite(Entity):
    ...
    def tick(self, time_step):  # computes
        ...
        self.next_satLat = wgs84.geographic_position_of(
            self.next_geocentric
        ).latitude.degrees
        self.next_satLon = wgs84.geographic_position_of(
            self.next_geocentric
        ).longitude.degrees
    ...
    def att_error(self, next_pos, next_vel):
        targetQuat = self.update_target_attitude(
            next_pos, next_vel, targetLoc, t_start, t_end
        )
