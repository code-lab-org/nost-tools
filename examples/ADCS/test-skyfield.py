...
# Define the nadir vector in the Earth-fixed frame
...
positionsGeo = np.zeros((num_steps, 3))
...
# Define the rotation rate of the spacecraft in the body-fixed frame
w = np.array([0, 0, 0.1])  # 0.1 rad/s about the body-fixed z-axis
...
for i in range(num_steps):
    ...
    # inertial position 
    geocentric = satellite.at(time).position.km    # position in GCRS/J2000
    positionsGeo[i] = geocentric
    ...
    h = np.cross(geocentric, velocity)
    
    posMag = np.linalg.norm(geocentric)            # position magnitude
    velMag = np.linalg.norm(velocity)            # velocity magnitude  
    b_x = velocity/velMag                        # body x-axis along velocity vector
    b_z = geocentric/posMag                        # body z-axis nadir pointing
