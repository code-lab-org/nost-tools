...
from delay_config_files.config import (
    PREFIX,
    APPS
)
...
def post_processing(names,msg_dictionaries,msg_periodicity=None, msg_size=None, wallclock_offset=None):
    """
    ...
    for i, application in enumerate(names):
        ...
        app_df_dict[application]['timeStamp'] = pd.to_datetime(app_df_dict[application]['timeStamp'])
        app_df_dict[application]['sysTimeNST'] = pd.to_datetime(app_df_dict[application]['sysTimeNST'])
        app_df_dict[application]['timeDiff (s)'] = (app_df_dict[application]['sysTimeNST'] - app_df_dict[application]['timeStamp']).dt.seconds
        app_df_dict[application]['timeDiff (microseconds)'] = (app_df_dict[application]['sysTimeNST'] - app_df_dict[application]['timeStamp']).dt.microseconds
        app_df_dict[application]['timeDiff total (ms)'] = app_df_dict[application]['timeDiff (s)']*1000.0 + (app_df_dict[application]['timeDiff (microseconds)']/1000.0)
        app_df_dict[application].loc[app_df_dict[application]["timeDiff (s)"]>=86399,"timeDiff total (ms)"] = app_df_dict[application]["timeDiff total (ms)"] - 86400000.0
        app_df_dict[application].loc[app_df_dict[application]["timeDiff (s)"]>=86399,"timeDiff (microseconds)"] = app_df_dict[application]["timeDiff total (ms)"]
        app_df_dict[application].loc[app_df_dict[application]["timeDiff (s)"]>=86399,"timeDiff (s)"] = 0
        ...
        statsDelay = {
                ...
                'simulation duration (s)': len(app_df_dict[application]['timeDiff total (ms)'])*msg_periodicity,
                ...
                'mode (ms)': stats.mstats.mode(app_df_dict[application]['timeDiff total (ms)'])[0][0],
                'standard deviation (ms)': np.std(app_df_dict[application]['timeDiff total (ms)']),
...
class HeartbeatDelayRecorder():
    ...
    def on_message(self, mqttc, obj, msg):
        ...
        if msg.topic == f"{PREFIX}/manager/start":
        ...
        elif msg.topic == f"{PREFIX}/heartbeat/settings":
        ...
        for i, application in enumerate(self.names):
            if msg.topic == f"{PREFIX}/status/{application}/time":
                ...
                if self.sim_end_time != None:
                    ...
                    if scenarioTime >= self.sim_end_time:
                        mqttc.disconnect
                        print("Simulation has passed the specified end time. Beginning post-processing:")
                        self.app_df_dict, self.stats_df_dict = post_processing(self.names,self.msg_dictionaries,self.msg_periodicity,self.msg_size,self._wallclock_offset)
                        self.app_df_dict['heartbeat'].to_csv(f"delay_data_collection/logs_msgFreq_{self.msg_periodicity}_msgSize_{self.msg_size}.csv")
                        if exists("delay_data_collection/doeStats.csv"):
                            self.stats_df_dict['heartbeat'].to_csv("delay_data_collection/doeStats.csv", mode='a',index=False,header=False)
                        else:
                            self.stats_df_dict['heartbeat'].to_csv("delay_data_collection/doeStats.csv", mode='a',index=False,header=True)
