...
.. toctree::
   ...
   modules/amqpProtocol
   modules/localBroker
   modules/publisher_consumer_example
   modules/nost_publisher_consumer_example
   
*NOTE:* This guide follows specific choices for a working testbed architecture, but many of these choices could be replaced by alternatives with similar functional capability. For example, NOS-T adopts the RabbitMQ event broker, but there are many other event broker products available that can also facilitate event-driven architectures. Similarly, Amazon Web Services (AWS) were chosen for hosting a cloud-based event broker, but other service providers and products may also satisfy the requirements for a cloud-based testbed architecture.